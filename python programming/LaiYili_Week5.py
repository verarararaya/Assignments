...
diamonds = DataFrame(pd.read_csv('Diamonds.csv'))
...
# Suggested test
print(exercise1(diamonds,np.arange(12),np.array([1,4,5,8])))
...
def exercise2(df,col,output_label,val1,val2):
    # df1 = df.loc[val1<col<val2]
    return df.loc[(val1<col)&(col<val2),output_label]
# Suggested test
test_df = exercise1(diamonds,np.arange(500),np.arange(10))
print(exercise2(test_df,test_df.carat,'price',1.1,1.4))
...
def exercise3(df):
    n1 = np.array(df.loc[:,['carat','table']])
    matrix = []
    for n in range(n1.shape[0]):
        lines = []
        for r in range(n1.shape[0]):
            sum = pow((n1[n,0]-n1[r,0])/0.8,2)+pow((n1[n,1]-n1[r,1])/57,2)
            lines.append(sum)
        matrix.append(lines)
...
def exercise4(df):
    ...
    for n in range(matrix.shape[0]):
        ...
        for r in range(matrix.shape[1]):
            sum = sum + matrix[n,r]
        sim.append(sum)
    df['Dissimilarity'] = sim
    return df
# Suggested test
print(exercise4(test_df_2))
